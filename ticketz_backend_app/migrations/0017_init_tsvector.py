...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.execute(
                   ...
                   CREATE OR REPLACE FUNCTION create_record_vector(record_id int) RETURNS text AS
                    $$
                        ...
                        BEGIN
                            tTitle:=(SELECT title from ticketz_backend_app_deal where id=record_id);
                            tDescription:=(SELECT description from ticketz_backend_app_deal where id=record_id);
                    ...
                    AS $$
                        DECLARE
                            r ticketz_backend_app_deal%%rowtype;
                        BEGIN
                            FOR r IN SELECT * FROM ticketz_backend_app_deal
                            LOOP
                                UPDATE ticketz_backend_app_deal SET search_index=to_tsvector(create_record_vector(r.id)) WHERE id=r.id;
                            END LOOP;
                        END;
                    $$ LANGUAGE plpgsql;
        ...
        db.execute(
                   """
                   CREATE OR REPLACE FUNCTION tsvector_deal_update_trigger_before() RETURNS TRIGGER AS $$
                    DECLARE
                        sVector VARCHAR;
                        r ticketz_backend_app_deal%%rowtype;
                        tTitle VARCHAR;
                ...
                CREATE TRIGGER tsvectorupdate_deal_before BEFORE INSERT OR UPDATE ON ticketz_backend_app_deal
                FOR EACH ROW EXECUTE PROCEDURE tsvector_deal_update_trigger_before();
    ...
    def backwards(self, orm):
        db.execute(
                   """
                       alter table ticketz_backend_app_deal DISABLE TRIGGER tsvectorupdate_schoolgroup_before ;
                       DROP FUNCTION tsvector_deal_update_trigger_before() CASCADE;
